...
//	implements Comparable<BigRational>
{
	 ...
	 * 
	 */
	private static final long serialVersionUID = -2768174971283489856L;
	...
	*/
	...
	/**Returns the least common denominator of the current BigRational and a given one.*/
	BigRational lcd(BigRational x) {
		return lcm(valueOf(x.m_q));
	}
	
	/**Returns the least common multiple of the current BigRational and a given one.*/
	BigRational lcm(BigRational x) {
		BigRational a = this;
		return a.multiply(x.divide(gcf(x)));
	}
	
	/**Returns the greatest common factor of the current BigRational and a given one.*/
	BigRational gcf(BigRational x) {
		x = x.abs();
		BigRational y = this.abs();
		while(!x.isZero()) {
			BigRational z = y.mod(x);
			y = x;
			x = z;
		}
		return y;
